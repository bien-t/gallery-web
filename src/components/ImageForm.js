...
function ImageForm({ onSubmit, onChange, imageUploadCloseForm, images, setImages }) {
    ...
    const showInputField = (event) => {
        ...
        setShowInput({
            ...showInput,
            [event.target.attributes['data-id'].value]: !showInput[event.target.attributes['data-id'].value]
        })
    }

    const updateName = (name, index) => (event) => {
        event.preventDefault()
    ...
    const update = (event) => {
        setImageName({
            ...imageName,
            [event.target.attributes['data-id'].value]: event.target.value
        })
    }
    ...
    return (
        <GalleryFormContainer>
            <Wrapper>
                ...
                <GalleryFormContent onSubmit={onSubmit} enctype="multipart/form-data">
                    <h2>Image details</h2>
